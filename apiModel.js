var apiModel = function() {
    ...
    var setTimeByDate = function(payload, callback) {
        function setTime(err, connection) {
            if (err) {
                callback({ code: 500, message: "There was an error while connecting to the database", err: err });
            } else {
                var insert = 'INSERT INTO `projects`.`time_entry` (`profileId`, `hours`, `timeCode`, `date`, `updaterId`) VALUES (';
                insert += connection.escape(payload.profileId) + ', ' + connection.escape(payload.hours) + ', ' + connection.escape(payload.timeCode) + ', ' + connection.escape(payload.date) + ', ' + connection.escape(payload.updaterId);
                insert += ')';
                console.log(insert)
